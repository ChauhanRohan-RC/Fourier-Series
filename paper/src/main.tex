...
\begin{document}
		...
		\subfloat[x-coordinate of COM vs $f_{w}$] {
			\begin{tikzpicture}
				...
				\begin{axis} [
					...
					\addplot[thin, smooth, color=red] table [header=true, col sep=comma, x=frequency, y=real] {res/data/2d_spring_wound_up_com.csv};
		...
		\subfloat[Distance of COM from origin vs $f_{w}$] {
			\begin{tikzpicture}
				...
				\begin{axis} [
					...
					\addplot[thin, smooth, color=red] table [header=true, col sep=comma, x=frequency, y=mag] {res/data/2d_spring_wound_up_com.csv};
		...
		\subfloat[Variation of distance of the COM from origin as wrapping frequency changes] {
			\begin{tikzpicture}
				...
				\begin{axis} [
					...
					\addplot[thin, smooth, color=red] table [header=true, col sep=comma, x=frequency, y=mag] {res/data/2d_deformed_spring_wound_up_com.csv};
	...
	\begin{figure}[H]
		...
		\subfloat[Point rotating in complex plane expressed as $e^{2\pi if_{w}t}$ with rotational frequency $f_{w}$]{
			\begin{tikzpicture}
				\begin{axis} [
					axis lines=middle, 
					xmin=-2.2, 
		...
		\subfloat[Wrapped up signal as the product of $f(t)$ and $e^{2\pi if_{w}t}$. Here $f_{w}=0.25$ cycles/s]{
			\begin{tikzpicture}
				\begin{axis} [
					...
					\draw[color=red, fill=red] (0,0) circle (1.7pt) node[above] {};
		...
		\subfloat[Wrapped up signal with frequency $f_{w}=2$ rotations/s]{
			\begin{tikzpicture}
				\begin{axis} [
					...
					\draw[color=red, fill=red] (0.5,0) circle (1.7pt) node[above] {};
					
				\end{axis}
			\end{tikzpicture}
		}
	
		\caption{Mathematical construct of wrapping a signal around a circle of unit radius in the complex plane with wrapping frequency $f_{w}$. The wrapped up signal can be traced by scaling a point rotating in complex plane (given by $e^{2\pi if_{w}t}$) by the signal $f(t)$ i.e $f(t)e^{2\pi if_{w}t}$ gives the wrapped signal. Center of mass of the wrapped up graph (shown as red dot) is given by the average of randomly sampled points along the curve (shown as green dots).}

...
\begin{document}
	...
	\begin{figure}[H]
		...
		\subfloat[$f_{w} = f_{t} = 2$ circular wraps/cm. \centering COM is unusally far from origin] {
			\begin{tikzpicture}
		...
		\subfloat[x-coordinate of COM vs $f_{w}$] {
			\begin{tikzpicture}
				...
				\begin{axis} [
					...
					\addplot[thin, smooth, color=red] table [header=true, col sep=comma, x=frequency, y=real] {res/data/2d_spring_4cm_2Hz_phase_180_wound_up_com.csv};
		...
		\subfloat[Distance of COM from origin vs $f_{w}$] {
			\begin{tikzpicture}
				...
				\begin{axis} [
					...
					\addplot[thin, smooth, color=red] table [header=true, col sep=comma, x=frequency, y=mag] {res/data/2d_spring_4cm_2Hz_phase_180_wound_up_com.csv};
	...
	This wrapping of a 2D spring around a circle for the extraction of frequency information also works for deformed springs. This is illustrated in figure \ref{fig:2d_deformed_spring_and_com_plot}. Consequently, a deformed 2D spring correlates with multiple frequencies (as in fig \ref{fig:wound_up_deformed_spring_com_distance_vs_wrap_freq}, it correlates highly with $2$ and $3$ wraps/cm), and is analogous to a signal made up of combination of multiple pure frequencies.
	
	\begin{figure}[H]
		\centering
		\subfloat[A 2D linear deformed spring]{
			\begin{tikzpicture}
				\label{fig:2d_linear_deformed_spring_turn_fq_2_3}
				\begin{axis} [
					axis lines=left, 
					xmin=0, 
					xmax=5, 
		...
		\subfloat[Variation of distance of the COM from origin as wrapping frequency changes] {
			\begin{tikzpicture}
				\label{fig:wound_up_deformed_spring_com_distance_vs_wrap_freq}
				\begin{axis} [
					axis lines=middle, 
					xmin=0, 
					xmax=5.6, 
		...
		\label{fig:2d_deformed_spring_and_com_plot}
	\end{figure}

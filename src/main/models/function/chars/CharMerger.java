package main.models.function.chars;
...
public class CharMerger extends CharFunction implements ColorHandler {
    ...
    private final double msDef, msMin, msMax;
    ...
    public CharMerger(@NotNull List<CharFunction> functions, float zoom, boolean center) {
        ...
        double _w = 0, _h = 0, _dEnd = 0, _msDef = 0, _msMin = 0, _msMax = 0;
        for (CharFunction f: functions) {
            _w += f.getWidth();
            _h = Math.max(_h, f.getHeight());
            _dEnd += f.getDomainEnd();
            _msDef += f.getDomainRangeTravelMsDefault();
            _msMin += f.getDomainRangeTravelMsMin();
            _msMax += f.getDomainRangeTravelMsMax();
        }
    ...
    public @NotNull Complex compute(double input) {
        ...
        while (itr.hasNext()) {
            ...
            if (input <= 1) {
                ...
                final Complex lerp = ComplexUtil.interpolateLinear(cur.compute(dend).add(w), next.compute(next.getDomainStart()).add(nextFuncWOffset), (float) input);

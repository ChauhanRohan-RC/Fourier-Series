...
import main.models.rotor.RotorStateProvider;
...
public class FourierPanel extends JPanel {
    ...
    public static final int MAX_WAVE_POINTS = 100000;
    ...
    public static final boolean AUTO_DISABLE_AUTO_TRACK_ON_END_PAUSE = false;

    @NotNull
    public static final EndBehaviour DEFAULT_END_BEHAVIOUR = EndBehaviour.REPEAT;
    ...
    public interface PanelListener {
        ...
        void onGraphInCenterChanged(boolean graphInCenter);

        void onRotorCountChanged(int rotorCount);
        ...
        default void onAutoTrackInCenterChanged(boolean autoTrackInCenter) { }
    }
    ...
    private final RotorStateManager.Listener mRotorStateListener = new RotorStateManager.Listener() {
        ...
        @Override
        public void onRotorCountChanged(@NotNull RotorStateProvider manager, int prevCount, int newCount) {
            FourierPanel.this.onRotorCountChanged(prevCount, newCount);
        }
    };
...
//    private final Map<Integer, RotorState> mRotorStates;
    ...
    private double mInputStepPerMs;
//    private double mAllRotorsMagnitudeSum = -1;
    private long mLastMainLoopTimeStamp = -1;

    private double mScale = 1;
//    @Nullable
//    private Point2D mScalePivot;
    @Nullable
    private Size mDrag;
    @NotNull
    ...
    @Nullable
    private Double mPendingStartInput;
    @NotNull
    private final Listeners<PanelListener> mPanelListeners = new Listeners<>();

//    private int mPendingRotorCount = -1;


    public FourierPanel(@NotNull RotorStateManager stateManager) {
//        checkRotorCount(initialRotorCount);

//        mRotorCount = ComplexUtil.constraint(MIN_ROTOR_COUNT, MAX_ROTOR_COUNT, initialRotorCount);
//        mRotorStates = new HashMap<>((int) (Math.max(16, initialRotorCount) * 1.4f));
...
//    public FourierPanel(@NotNull RotorStateManager stateManager) {
//        this(stateManager, stateManager.getDefaultInitialRotorCount());
//    }
    ...
    public void addPanelListener(@NotNull FourierPanel.PanelListener l) {
        mPanelListeners.addListener(l);
    }

    public boolean removePanelListener(@NotNull FourierPanel.PanelListener l) {
    ...
    protected void onInputStepPerMsChanged(double stepPerMs) {
        final int p = mRotorStateManager.domainStepPerMsToPercent(stepPerMs);
        forEachPanelListener(l -> l.onDomainTravelSpeedChanged(p));
    }

    private double setInputStepPerMsInternal(double newStepPerMs) {
        if (mInputStepPerMs != newStepPerMs) {
            mInputStepPerMs = newStepPerMs;
            onInputStepPerMsChanged(newStepPerMs);
        }

        return mInputStepPerMs;
    }

    public final double setDomainTravelSpeedPercentage(int percent) {
        final double step = mRotorStateManager.percentToDomainStepPerMs(percent);
        return setInputStepPerMsInternal(mInputStepPerMs < 0? -step: step);
    }

    public final int getDomainTravelSpeedPercent() {
        return mRotorStateManager.domainStepPerMsToPercent(mInputStepPerMs);
    }


    protected void onRotorCountChanged(int prevCount, int newCount) {
        resetInput(false);
        invalidateRotorInitialStates(true);     // can depend upon count
        invalidateWave();
        update();

        forEachPanelListener(l -> l.onRotorCountChanged(newCount));
    }


//    private void doSetRotorCountInternal(int rotorCount) {
//        // Have to load states
//        if (mPendingRotorCount >= rotorCount && mRotorStateManager.isLoading())
//            return;
//
//        mPendingRotorCount = rotorCount;
//
//        if (!mRotorStateManager.hasListener(mLoadListener)) {
//            mRotorStateManager.addListener(mLoadListener);
//        }
//
//        mRotorStateManager.loadAsync(rotorCount);
//    }
    ...
    private void syncRotorStateManagerInternal(boolean update) {
        if (!mRotorStateManager.hasListener(mRotorStateListener)) {
            mRotorStateManager.addListener(mRotorStateListener);
        }

        resetInput(false);
        invalidateRotorInitialStates(true);
        invalidateWave();
        setInputStepPerMsInternal(mRotorStateManager.getDomainStepPerMsDefault());

        mRotorStateManager.considerInitialize();

//        resetInput(false);

//        final int defaultRotorCount = mRotorStateManager.getDefaultInitialRotorCount();
//        if (mRotorCount < defaultRotorCount) {
//            setRotorCount(defaultRotorCount);
//        } else {
//            mPendingRotorCount = -1;        // invalidate
//            doSetRotorCountInternal(mRotorCount);       // sync new rotors
//        }
...
//        RotorState state = mRotorStates.get(index);
//        if (state == null) {
//            state = retrieveRotorStateImpl(index, count);
//            mRotorStates.put(index, state);
//        }
//
//        return state;
    ...
    // Costly
    protected final void invalidateRotorInitialStates(boolean hard) {
//        if (hard) {
//            mRotorStates.clear();
//        }

//        mAllRotorsMagnitudeSum = -1;
    }
    ...
    public void resetInput(boolean update) {
        mPendingStartInput = null;
        mInput = mRotorStateManager.getDomainStart();
...
//        final boolean changed = pivotChanged || scaleChanged;
    ...
    public final void reset(boolean resetScaleAndDrag) {
        resetInput(false);
        invalidateRotorInitialStates(false);
        invalidateWave();
    ...
    protected Point2D parseWavePoint(int index, @NotNull Complex o, double baseRadius) {
        return new Point2D.Double(mDrawAsWave? index: transformX(o.getReal()) * baseRadius, transformY(o.getImaginary()) * baseRadius);
    }
...
//    protected double getTipRadiusPix(int rotorIndex, int rotorCount, boolean graphingInCenter) {
//        return graphingInCenter? DEFAULT_POINT_RADIUS_PIX_CENTER: DEFAULT_POINT_RADIUS_PIX;
//    }
...
//    protected final double getAllRotorsMagnitudeScaleSum(int rotorCount, int firstMultiplier) {
//        if (mAllRotorsMagnitudeSum > 0)
//            return mAllRotorsMagnitudeSum;
//
//        double r = 0;
//
//        if (rotorCount > 0) {
...
//        Log.v("Frame Drawn in " + (System.currentTimeMillis() - startMs) + "ms");
//
        final boolean shouldDrawRotors = shouldDrawRotors();
        final double input = mInput;
...
//            final double tipRadius = getTipRadiusPix(i, count, graphingInCenter);
//            g.fill(new Ellipse2D.Double(transformX(sum.getReal()) - tipRadius, transformY(sum.getImaginary()) - tipRadius, tipRadius * 2, tipRadius * 2));
    ...
    protected void onEndPause() {
        if (AUTO_DISABLE_AUTO_TRACK_ON_END_PAUSE && isAutoTrackingInCenter()) {
            setAutoTrackInCenter(false);
        }
    }

    protected void onEndRepeat() {

    }

    protected void onEndCycle() {
    ...
    protected void mainLoop() {
        ...
        if (pTimeMs != -1) {
            mInput += (mInputStepPerMs * (cTimeMs - pTimeMs));

            final DomainProviderI.EndInfo endInfo = mRotorStateManager.isEnded(mInputStepPerMs >= 0, mInput);
            if (endInfo.ended) {
                switch (mEndBehaviour) {
                    case PAUSE -> {
                        mInput = endInfo.endBound;
                        setPlay(false);
                        onEndPause();
                    } case REPEAT -> {
                        mInput = endInfo.isBoundDomainEnd? mRotorStateManager.getDomainStart(): mRotorStateManager.getDomainEnd();      // start back
                        onEndRepeat();
                    } case CYCLE -> {
                        mInput = endInfo.endBound;
                        mInputStepPerMs *= -1;
            ...
            if (mPendingStartInput != null) {
                mInput = mPendingStartInput;
                mPendingStartInput = null;
            } else {
                mInput = mRotorStateManager.getDomainStart();
            }
    ...
    // Call after pausing main loop
    private void noteCurrentInputOnPause() {
        mPendingStartInput = mInput;
        mLastMainLoopTimeStamp = -1;
    }
    ...
    protected void onEndBehaviourChanged(@NotNull EndBehaviour old, @NotNull EndBehaviour _new) {
        forEachPanelListener(l -> l.onEndBehaviourChanged(old, _new));
    }


    public final void setEndBehaviour(@NotNull EndBehaviour endBehaviour) {
    ...
    public final FourierUi getFourierUI() {
        final Window w = SwingUtilities.windowForComponent(FourierPanel.this);

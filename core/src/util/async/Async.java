...
public class Async {
    ...
    /**
     * Thread Pool Executor with unlimited workers
     * */
    public static final ExecutorService THREAD_POOL_EXECUTOR = Executors.newFixedThreadPool(10);
...
//    }
    ...
    public static void uiPost(@NotNull Runnable runnable, int delayMs) {
        if (delayMs <= 0) {
            ...
            final javax.swing.Timer t = new Timer(delayMs, e -> runnable.run());
            t.setRepeats(false);
            t.start();
        }
    }

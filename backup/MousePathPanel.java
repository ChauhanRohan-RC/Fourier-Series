...
//    private final List<Path2D> mPaths = new LinkedList<>();
    ...
    @NotNull
    private final LinkedList<LinkedList<Point2D>> mUndoStack = new LinkedList<>();
...
//        path.moveTo(anchor.getX(), anchor.getY());
    ...
    public void clear(boolean saveInStack) {
//        mCurPath = null;
//        mPaths.clear();
        ...
        if (saveInStack) {
            ...
            LinkedList<Point2D> p;
            while ((p = copy.pollLast()) != null) {
                mUndoStack.add(p);
            }
    ...
    @Nullable
    public LinkedList<Point2D> removeLastPath(boolean saveInStack) {
    ...
    @Nullable
    public LinkedList<Point2D> undo() {
        return removeLastPath(true);
    }

    @Nullable
    public LinkedList<Point2D> redo() {
        final LinkedList<Point2D> last = mUndoStack.pollLast();
        if (last == null)
            return null;
...
//            }
    ...
    public BaseAction getUndoAction() {
        ...
        if (a == null) {
            synchronized (this) {
                a = mUndoAction;
                if (a == null) {
                    a = new RemoveLastPathAction(true);
                    mUndoAction = a;
                }
            }
        }
    ...
    private class RemoveLastPathAction extends BaseAction {

        private final boolean saveInStack;

        private RemoveLastPathAction(boolean saveInStack) {
            this.saveInStack = saveInStack;

            this.setName(saveInStack? "Undo": "Remove");
            this.setShortDescription(saveInStack? "Undo [ctrl-Z]": "Remove last path");
